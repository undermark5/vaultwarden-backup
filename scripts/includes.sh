...
function send_discord_webhook() {
    ...
    local ERROR=$1
    if [[ -z "$1" ]]; then
        /app/discord.sh --username "Vaultwarden Backup Bot" --description "Vaultwarden Backup Run Complete" --color "0x0d691a" --field "Status;Success;false"
    else
        /app/discord.sh --username "Vaultwarden Backup Bot" --description "Vaultwarden Backup Run Complete" --color "0x69130d" --field "Status;Failed;false" --field "Error;$1"
    fi
}
...
function send_discord_test() {
    ...
    /app/discord.sh --username "Vaultwarden Backup Bot" --description "Vaultwarden Webhook Test" --color "0x0d691a" --field "Status;Test Success;false"
}
